...
def select_action(epsilon, observation):
    ...
    if r > epsilon:
        ...
        ind = np.argmax(observation)
        priority_factor = 0.2
        if ind < 3:
            if a_probs[0] + priority_factor < 1:
                a_probs[0] = a_probs[0] + priority_factor
        elif ind > 3:
            if a_probs[1] + priority_factor < 1:
                a_probs[1] = a_probs[1] + priority_factor
        else:
            if a_probs[2] + priority_factor < 1:
                a_probs[2] = a_probs[2] + priority_factor
...
class DQNAgent:
    ...
    def train(self, terminal_state, step ):
        ...
        for index, (current_state, action, reward, new_current_state, done) in enumerate(minibatch):
            ...
            current_q = self.model.predict_on_batch(np.reshape(current_state, (1, 7, 1))) # This has 3 q-values (for both the action was taken and not)
            print(current_q)
            # target = reward
            if not done:
